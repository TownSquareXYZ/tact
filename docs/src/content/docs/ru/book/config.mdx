...
Файл [JSON-схемы](https://json-schema.org/) доступен редакторам для автоматического заполнения и подсказок при наведении: [configSchema.json](http://raw.githubusercontent.com/tact-lang/tact/main/schemas/configSchema.json).
...
Если установлено значение `true{:json}`, включается отладочный вывод контракта и разрешается использование функции [`dump(){:tact}`](/ref/core-debug#dump), которая полезна для [целей отладки](/book/debug). Если эта опция включена, контракт сообщит, что он был скомпилирован в режиме отладки с использованием метода `supported_interfaces`.
...
:::note
  ...
  Подробнее читайте на специальной странице: [Отладка](/book/debug).
...
Если установлено значение `true{:json}`, включается поддержка [мастерчейна](/book/masterchain).
...
:::note
  ...
  Подробнее читайте на специальной странице: [Мастерчейн](/book/masterchain).
...
Если установлено значение `true{:json}`, включает поддержку получения [внешних](/book/external) сообщений.
...
:::note
  ...
  Подробнее читайте на специальной странице: [Внешние сообщения](/book/external).
...
Если установлено значение `true{:json}`, включает генерацию функции [геттер](/book/contracts#getter-functions) со ссылками IPFS, описывающими ABI контракта.
...
:::note
  ...
  Подробнее читайте на специальной странице: [OTP-003: Самостоятельный отчет ABI](/ref/evolution/otp-003).
...
Если установлено значение `true{:json}`, включает генерацию [getter](/book/contracts#getter-functions) со списком интерфейсов, предоставляемых контрактом.
...
:::note
  ...
  Подробнее: [Поддерживаемые интерфейсы](/book/contracts#interfaces).
