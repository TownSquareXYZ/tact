...
Помимо дампа значений, часто бывает полезно использовать утвердительные функции, такие как [`require(){:tact}`](/ru/ref/core-debug#require), [`nativeThrowIf(){:tact}`](/ru/ref/core-debug#nativethrowif) и [`nativeThrowUnless(){:tact}`](/ru/ref/core-debug#nativethrowunless). Они помогают четко сформулировать ваши предположения и удобны для настройки "растяжек" для обнаружения проблем в будущем.
...
Tact предоставляет несколько различных функций, полезных для отладки: [Основная библиотека → Отладка](/ru/ref/core-debug).

## Включение режима отладки в параметрах компиляции {#debug-mode}

Чтобы заставить работать определенные функции, такие как [`dump(){:tact}`][dump] или [`dumpStack(){:tact}`](/ru/ref/core-debug#dumpstack), необходимо включить режим отладки.

Самый простой и рекомендуемый подход — изменить файл [`tact.config.json`](/ru/book/config) в корне вашего проекта (или создать его, если он еще не существует) и [установить свойство `debug` в `true{:json}`](/ru/book/config#options-debug).
...
Кроме того, [`tact.config.json`](/ru/book/config) по-прежнему можно использовать в проектах [Blueprint][bp]. В таких случаях значения, указанные в [`tact.config.json`](/ru/book/config), действуют как значения по умолчанию, если они не изменены в `wrappers/`.
...
:::note
  ...
  Узнайте больше о конфигурации и файле [`tact.config.json`](/ru/book/config): [Конфигурация](/ru/book/config).\
...
:::note
  ...
  Настоятельно рекомендуется не изменять содержимое `beforeEach(){:tact}`, если только вам действительно не нужно определенное поведение для каждого завершения теста или параметры вашей функции [`init(){:tact}`](/ru/book/contracts#init-function) не изменились.
...
:::note
  ...
  Вызов [`dump(){:tact}`][dump] требует больших вычислительных затрат и потребляет много газа. Обязательно всегда предоставляйте достаточное количество Toncoin в своих тестах, чтобы избежать [кода выхода -14](/ru/book/exit-codes#-14).
...
Предположим, что вы создали [новый проект контр-контракта](/ru/#start), давайте посмотрим, как это работает на практике.
...
Он отправляет сообщение нашему контракту `receive(msg: Add){:tact}` [получателю](/ru/book/receive) без сохранения [результатов такой отправки](#tests-send).
...
Метод `expect(…).toEqualCell(){:typescript}` проверяет равенство двух [ячеек](/ru/book/cells#cells):
...
Метод `expect(…).toEqualSlice(){:typescript}` проверяет равенство двух [срезов](/ru/book/cells#slices):
...
Метод `expect(…).toEqualAddress(){:typescript}` проверяет равенство двух [адресов](/ru/book/types#primitive-types):
...
Тело сообщения может быть простой строкой или объектом, определяющим поля типа [сообщение](/ru/book/structs-and-messages#messages):
...
[Sandbox][sb] предоставляет вспомогательную функцию `printTransactionFees(){:typescript}`, которая наглядно выводит все значения и комиссии, которые были внесены в предоставленные транзакции. Это очень удобно для наблюдения за потоком [nanoToncoin](/ru/book/integers#nanotoncoin).
...
Иногда полезно проводить отрицательные тесты, показывая преднамеренные ошибки и выдавая определенные [коды выхода](/ru/book/exit-codes).
...
[Глобальная статическая функция](/ru/book/functions#global-static-functions) [`emit(){:tact}`](/ru/ref/core-common#emit) отправляет сообщение во внешний мир — у нее нет конкретного получателя.

Эта функция очень удобна для автономного ведения журнала и анализа данных - достаточно просто просмотреть [внешние сообщения](/ru/book/external), созданные контрактом.

### Журналы в локальных тестах Sandbox {#logging-local}

При развертывании в [Sandbox][sb] вы можете вызвать [`emit(){:tact}`](/ru/ref/core-common#emit) из [функции-получателя](/ru/book/contracts#receiver-functions), а затем просмотреть список отправленных [внешних сообщений](/ru/book/external):
...
Чтобы увидеть журналы из [`emit(){:tact}`](/ru/ref/core-common#emit) в этом словаре, найдите внешние сообщения без получателя. В различных обозревателях блокчейна TON такие транзакции будут помечены как `external-out` с указанием назначения как `-` или `empty`.
...
Там объект `res` будет содержать список отправленных [внешних сообщений](/ru/book/external) в качестве поля `externals`. Давайте получим к нему доступ, чтобы проанализировать первое сообщение, отправленное через вызов [`emit(){:tact}`](/ru/ref/core-common#emit) в коде Tact (или _emitted_ для краткости):
...
При [отправке](/ru/book/send) с `bounce: true{:tact}` сообщения могут возвращаться обратно в случае ошибок. Обязательно напишите соответствующие [`bounced(){:tact}`](/ru/book/bounced) сообщения [получателям](/ru/book/contracts#receiver-functions) и корректно обрабатывайте возвращенные сообщения:
...
Подробнее об возвращенных сообщениях и получателях: [Возвращенные сообщения](/ru/book/bounced).
...
3. #### Измените контракт {#lab-3}
   ...
   Основная идея этой настройки заключается в том, чтобы поместить код, который вы хотите протестировать, в [функцию-получатель](/ru/book/contracts#receiver-functions), отвечающую на [строковое](/ru/book/types#primitive-types) сообщение `"plays"{:tact}`.

   Обратите внимание, что вы по-прежнему можете написать любой допустимый код Tact за пределами этого [приемника](/ru/book/contracts#receiver-functions). Но чтобы протестировать его, вам нужно будет написать соответствующую тестовую логику внутри него.
...
4. #### Давайте протестируем! {#lab-4}
   ...
   С этого момента, чтобы что-то протестировать, вам нужно будет только изменить содержимое тестируемой [функции-получателя](/ru/book/contracts#receiver-functions) вашего файла контракта Tact и повторно запустить команду выше. Измените и повторите этот процесс, пока не протестируете то, что хотели протестировать.
