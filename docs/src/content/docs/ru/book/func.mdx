...
[Примитивные типы](/book/types#primitive-types) в Tact напрямую преобразуются в типы FunC.

Все правила копирования переменных одинаковы. Одно из главных отличий заключается в том, что в Tact нет видимых операторов мутации, и большинство операций [`Slice{:tact}`](/book/cells#slices) изменяют переменные на месте.

## Преобразование сериализации

Сериализация [структур](/book/structs-and-messages#structs) и [сообщений](/book/structs-and-messages#messages) в Tact происходит автоматически, в отличие от FunC, где вам нужно вручную определять логику сериализации.
...
Все необязательные типы сериализуются как `Maybe` в TL-B, за исключением [`Address{:tact}`](/book/types#primitive-types).
...
## Преобразование полученных сообщений в операции `op`
...
Чтобы преобразовать тензор, который возвращается из метода `get` FunC, вам нужно определить [структуру](/book/structs-and-messages#structs), которая имеет те же типы полей, что и тензор, и в том же порядке.
...
Когда некоторые из тензоров являются кортежем, вам нужно определить структуру, как на предыдущих шагах, а кортеж должен быть определен как отдельная [структура](/book/structs-and-messages#structs).
...
Преобразование аргументов методов `get` выполняется просто. Каждый аргумент сопоставляется _как есть_ на один из аргументов FunC, а каждый кортеж сопоставляется со [структурой](/book/structs-and-messages#structs).
