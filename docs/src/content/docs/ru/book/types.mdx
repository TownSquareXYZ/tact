...
- [`Int{:tact}`](/book/integers) — все числа в Tact являются $257$-битными целыми числами со знаком, но [более мелкие представления](/book/integers#serialization) могут использоваться для снижения затрат на хранение.
...
- [`Cell{:tact}`](/book/cells#cells), [`Builder{:tact}`](/book/cells#builders), [`Slice{:tact}`](/book/cells#slices) — низкоуровневые примитивы [TVM][tvm].
...
В Tact нет неявных преобразований типов, поэтому сложение ([`+{:tact}`](/book/operators#binary-add)) двух логических значений невозможно. Кроме того, доступно множество сравнительных [операторов](/book/operators), таких как:

- `&&{:tact}` для [логического И](/book/operators#binary-logical-and),
- `||{:tact}` для [логического ИЛИ](/book/operators#binary-logical-or),
- `!{:tact}` для [логической инверсии](/book/operators#unary-inverse),
- `=={:tact}` и `!={:tact}` для проверки [равенства](/book/operators#binary-equality),
- и `!!!{:tact}` для [ненулевого утверждения](/book/optionals).
...
В дополнение к вышеперечисленным составным типам Tact предоставляет специальный конструктор типа [`bounced<T>{:tact}`](/book/bounced), который может быть указан только в [получателях возвращенных сообщений](/book/bounced).

Обратите внимание, что хотя [контракты](#contracts) и [признаки](#traits) также считаются частью системы типов Tact, их нельзя передавать, как [структуры и сообщения](#structs-and-messages). Вместо этого можно получить начальное состояние данного контракта, используя выражение [`initOf{:tact}`](/book/expressions#initof).
...
[с-н-м]: /book/structs-and-messages
[Структуры]: /book/structs-and-messages#structs
[сообщения]: /book/structs-and-messages#messages
...
[факультативы]: /book/optionals

### Контракты

[Контракты](/book/contracts) в Tact служат основными точками входа смарт-контрактов блокчейна TON. Они содержат все [функции](/book/functions), [получатели](/book/functions#getter-functions) и [приемники](/book/functions#receiver-functions) контракта TON и многое другое.

Пример [контракта](/book/contracts):
...
Подробнее об этом на отдельной странице: [контракты](/book/contracts).

### Типажи

Tact не поддерживает классическое наследование классов, но вместо этого вводит концепцию _типажей_, которые можно рассматривать как абстрактные контракты (подобно абстрактным классам в популярных объектно-ориентированных языках). Они имеют ту же структуру, что и [контракты](#contracts), но не могут [инициализировать постоянные состояния переменных](/book/contracts#init-function).

Типаж также может позволить наследующему ее контракту переопределять поведение его [функций](/book/functions#virtual-and-abstract-functions) и значение его [констант](/book/constants#virtual-and-abstract-constants).

Пример типажа [`Ownable{:tact}`](/ref/stdlib-ownable#ownable) из [`@stdlib/ownable`](/ref/stdlib-ownable):
...
И [contract](#contracts), который использует типаж [`Ownable{:tact}`](/ref/stdlib-ownable#ownable):
