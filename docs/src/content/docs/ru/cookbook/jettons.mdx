...
Используйте функцию [receiver](/book/receive), чтобы принять сообщение с уведомлением о токене.
...
Вы можете получить адрес кошелька Jetton контракта, используя функцию [`contractAddress(){:tact}`](/ref/core-common#contractaddress) или вычислить этот адрес offchain.
...
Поскольку уведомления поступают из кошелька Jetton вашего контракта, в поле `ownerAddress` следует использовать функцию [`myAddress(){:tact}`](/ref/core-common#myaddress).
...
Для отправки перевода Jetton используйте функцию [`send(){:tact}`](/book/send).
