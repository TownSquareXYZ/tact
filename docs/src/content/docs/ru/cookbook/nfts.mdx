...
## Принятие передачи права собственности на NFT {#accepting-nft-ownership-assignment}
...
Для принятия уведомления используйте функцию [receiver](/ru/book/receive).
...
2. Используйте [`StateInit{:tact}`](/ru/book/expressions#initof) и производный адрес элемента NFT.
...
Для передачи NFT используйте функцию [`send(){:tact}`](/ru/book/send).
