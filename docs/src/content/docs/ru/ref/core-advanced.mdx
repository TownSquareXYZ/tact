...
Обратите внимание, что значения `cell` и `bits` берутся по модулю их максимальных значений плюс $1$. То есть указание значений, превышающих указанные в [пределах состояния счета (`max_acc_state_cells` и `max_acc_state_bits`)](/ru/book/exit-codes#50), даст тот же результат, что и указание точных пределов. Кроме того, убедитесь, что вы учитываете [дедупликацию ячеек с одинаковым хешем][deduplication].

Попытки указать отрицательное число `cell`, `bits` или `seconds` вызывают исключение с [кодом выхода 5](/ru/book/exit-codes#5): `Целое число вне ожидаемого диапазона`.
...
Попытки указать отрицательное значение `gasUsed` вызывают исключение с [кодом выхода 5](/ru/book/exit-codes#5): `Целое число вне ожидаемого диапазона`.
...
Функция расширения для [`Context{:tact}`](/ru/ref/core-common#context).
...
Обратите внимание, что значения `cells` и `bits` берутся по модулю их максимальных значений плюс $1$. То есть указание значений, превышающих указанные в [лимитах состояния аккаунта (`max_msg_cells` и `max_msg_bits`)](/ru/book/exit-codes#50), даст тот же результат, что и указание точных ограничений.
...
Попытки указать отрицательное количество `cells` или `bits` вызовут исключение с [кодом выхода 5](/ru/book/exit-codes#5): `Целое число вне ожидаемого диапазона`.
...
Попытки указать отрицательное количество `cell` или `bits` вызывают исключение с [кодом выхода 5](/ru/book/exit-codes#5): `Целое число вне ожидаемого диапазона`.
...
Попытки указать отрицательное значение `fwdFee` вызывают исключение с [кодом выхода 5](/ru/book/exit-codes#5): `Целое число вне ожидаемого диапазона`.
...
:::note
  ...
  Стандартная библиотека [`@stdlib/config`](/ru/ref/stdlib-config) предоставляет две связанные вспомогательные функции:\
[`getConfigAddress(){:tact}`](/ru/ref/stdlib-config#getconfigaddress) для получения конфигурации [`Address{:tact}`][p]\
[`getElectorAddress(){:tact}`](/ru/ref/stdlib-config#getconfigaddress) для получения выборщика [`Address{:tact}`][p]
...
Подготавливает генератор случайных чисел с помощью [`nativeRandomizeLt(){:tact}`](#nativerandomizelt). Автоматически вызывается функциями [`randomInt(){:tact}`](/ru/ref/core-random#randomint) и [`random(){:tact}`](/ru/ref/core-random#random).
...
Генерирует и возвращает $256$-битное случайное число, как [`randomInt(){:tact}`](/ru/ref/core-random#randomint), но не инициализирует генератор случайных чисел с помощью [`nativePrepareRandom(){:tact}`](#nativepreparerandom) заранее.

:::note

  Не используйте эту функцию напрямую и предпочитайте использовать [`randomInt(){:tact}`](/ru/ref/core-random#randomint).
...
Генерирует и возвращает $256$-битное случайное число в диапазоне от $0$ до `max`, как [`random(){:tact}`](/ru/ref/core-random#random), но не инициализирует генератор случайных чисел с помощью [`nativePrepareRandom(){:tact}`](#nativepreparerandom) заранее.

:::note

  Не используйте эту функцию напрямую, а вместо этого используйте [`random(){:tact}`](/ru/ref/core-random#random).
...
[Помещает сообщение](/ru/book/send#outbound-message-processing) в очередь для отправки, указав полную `cell` и [`mode` сообщения](/ru/book/message-mode).
...
:::note
  ...
  Предпочитайте использовать гораздо более распространенную и удобную функцию [`send(){:tact}`](/ru/ref/core-common#send), если только у вас нет сложной логики, которую нельзя выразить иным образом.
...
:::caution
  ...
  Кроме того, попытки поставить в очередь бронирования на сумму более $255$ за одну транзакцию вызывают исключение с [кодом выхода 33](/ru/book/exit-codes#33): `Список действий слишком длинный`.
...
Чтобы сделать значение [`Int{:tact}`][int] для параметра `mode`, вам просто нужно объединить базовые режимы с дополнительными флагами, применив операцию [побитовое ИЛИ](/ru/book/operators#binary-bitwise-or):
...
Попытки передать [`Slice{:tact}`][slice] с расположением, отличным от `StdAddress{:tact}`, или загрузить больше данных, чем задано[`Slice{:tact}`][slice], содержит исключение с [кодом выхода 9](/ru/book/exit-codes#9): `Переполнение ячейки`.
...
Попытки передать [`Slice{:tact}`][slice] с макетом, отличным от `VarAddress{:tact}`, или загрузить больше данных, чем задано в [`Slice{:tact}`][slice], вызывают исключение с [кодом выхода 9](/ru/book/exit-codes#9): `Переполнение ячейки`.
...
[кусочек]: /ru/book/cells#slices
[с]: /ru/book/structs-and-messages#structs
[шедевр]: https://docs.ton.org/v3/documentation/smart-contracts/shards/shards-intro#masterchain
[хеш ячейки]: /ru/ref/core-cell#cellhash
[nanotoncoin]: /ru/book/integers#nanotoncoin
