...
Каждый [контракт](/book/contracts) и [типаж](/book/types#traits) в Tact неявно [наследует](/book/contracts#traits) типаж `BaseTrait{:tact}`, который содержит ряд наиболее полезных [внутренних функций](/book/contracts#internal-functions) для любого типа контракта, а также константу `self.storageReserve{:tact}`, предназначенную для продвинутых пользователей Tact.
...
[Помещает сообщение](/book/send#outbound-message-processing) (с возможностью возврата или без возможности возврата) в очередь на отправку по указанному адресу `to`. При желании вы можете указать `тело` сообщения и пакет [`init`](/book/expressions#initof).
...
Чтобы иметь возможность отправить больше [nanoToncoin][nano] одним сообщением, используйте функцию [`send(){:tact}`](/ref/core-common#send).
