...
Проверяет `condition` и выдает ошибку с [кодом выхода](/ru/book/exit-codes), сгенерированным из сообщения `error`, если `condition` равно `false{:tact}`. В ином случае ничего не делает.
...
Сгенерированный код выхода гарантированно находится за пределами общего диапазона $0 - 255$, зарезервированного для ошибок контракта TVM и Tact, что позволяет отличать коды выхода от `require(){:tact}` и любых других [стандартных кодов выхода](/ru/book/exit-codes).
...
Выводит аргумент `arg` в отладочную консоль контракта. Оценивается только в том случае, если параметр `debug` в [файле конфигурации](/ru/book/config) установлен в `true{:json}`, в противном случае ничего не делает.
...
- [`map<K, V>{:tact}`](/ru/book/maps)
- [Необязательные и значение `null{:tact}`](/ru/book/optionals)
...
:::note[Useful ссылки:]
  ...
  [Отладка с помощью `dump(){:tact}`](/ru/book/debug#tests-dump)
...
Выводит общую глубину стека и до $255$ его значений сверху в отладочную консоль контракта. Значения располагаются снизу вверх: от самого глубокого значения слева до самого верхнего значения справа. Оценивается только в том случае, если параметр `debug` в [файле конфигурации](/ru/book/config) установлен в `true{:json}`, в противном случае ничего не делает.
...
:::note[Useful Полезные ссылки:]
  ...
  [Отладка с помощью `dump(){:tact}`](/ru/book/debug#tests-dump)\
[Функции сборки](/ru/book/assembly-functions)
...
Выдает исключение с кодом ошибки, равным `code`. Выполнение текущего контекста останавливается (операторы после `nativeThrow` не будут выполнены), и управление будет передано первому блоку [`try...catch{:tact}`](/ru/book/statements#try-catch) в стеке вызовов. Если среди вызывающих функций нет блока `try{:tact}` или `try...catch{:tact}`, [TVM](https://docs.ton.org/learn/tvm-instructions/tvm-overview) завершит транзакцию.

Попытки указать `code` за пределами диапазона $0 - 65535$ вызовут исключение с [кодом выхода 5](/ru/book/exit-codes#5): `Целое число вне ожидаемого диапазона`.
...
Попытки указать `code` за пределами диапазона $0 - 65535$ вызывают исключение с [кодом выхода 5](/ru/book/exit-codes#5): `Целое число выходит за пределы ожидаемого диапазона`.
...
[строитель]: /ru/book/cells#builders
