...
Чтобы использовать литералы [`String{:tact}`][p] напрямую, см.: [Литералы строки](/ru/book/expressions#string-literals).
...
Возвращает собранную [`Cell{:tact}`][cell] как [`Slice{:tact}`][slice] из [`StringBuilder{:tact}`][p]. Псевдоним для [`self.toCell().asSlice(){:tact}`](/ru/ref/core-cells#cellasslice).
...
:::note
  ...
  Посмотрите, как функция `String.asSlice{:tact}` может использоваться на практике: [Как преобразовать `String` в `Int`](/ru/cookbook/type-conversion#how-to-convert-a-string-to-an-int).
...
Если заданный [`String{:tact}`][p] содержит символы не из набора Base64, будет выдано исключение с [кодом выхода 134](/ru/book/exit-codes#134): `Недопустимый аргумент`.
...
Если заданный [`Slice{:tact}`][p] содержит символы не из набора Base64, будет выдано исключение с [кодом выхода 134](/ru/book/exit-codes#134): `Недопустимый аргумент`.
...
Точнее, `digits` — это параметр возведения в степень выражения $10^{-\mathrm{digits}}$, который дает представленное дробное число при умножении на фактическое значение [`Int{:tact}`][int]. Параметр `digits` должен находиться в замкнутом интервале: $0 <$ `digits` $< 78$, в противном случае будет выдано исключение с [кодом выхода 134](/ru/book/exit-codes#134): `Недопустимый аргумент`.
...
Используется для представления значений [nanoToncoin](/ru/book/integers#nanotoncoin) [`Int{:tact}`][int] с помощью строк.
