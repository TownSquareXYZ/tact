---
title: Спецификация Tact
description: Грамматика Tact, используемая в компиляторе, написана на языке Ohm, который основан на грамматиках разбора выражений (PEGs) - формальном способе описания синтаксиса, аналогичном регулярным выражениям и контекстно-свободным грамматикам.
pagefind: false
---

:::danger[Not Не реализовано]
  Эта страница является временной, пока не будет реализована в [#76](https://github.com/tact-lang/tact-docs/issues/76)
:::

Грамматика Tact, используемая в компиляторе, написана на [языке Ohm](https://ohmjs.org), который основан на [грамматиках разбора выражений](http://en.wikipedia.org/wiki/Parsing_expression_grammar) (PEGs) — формальном способе описания синтаксиса, аналогичном регулярным выражениям и контекстно-свободным грамматикам.

{/*
    Это комментарий, и он скрыт на странице.

```
Shiki плохо справляется с подсветкой Ohm через грамматику TextMate, хотя в VSCode это работает хорошо. Думаю, причина может заключаться в том, что Shiki подсвечивает строки по одной или использует другой движок RegEx. Я не уверен, в чем проблема, поэтому до ее исправления лучше использовать tree-sitter-ohm и генерировать ANSI-последовательности с помощью команды `highlight`. Остальное сделает Shiki:
```
