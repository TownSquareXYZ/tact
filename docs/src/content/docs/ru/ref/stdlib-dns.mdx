...
Преобразует строку DNS в [`Slice{:tact}`][slice] или [`null{:tact}`](/book/optionals), если это невозможно.
...
Нормализует внутреннее представление DNS [`Slice{:tact}`][slice]. Переданный [`Slice{:tact}`][slice] не должен иметь никаких ссылок, в противном случае будет выдано исключение с [кодом выхода 134](/book/exit-codes#134): `Недопустимый аргумент`.
...
1. [функция получатель](/book/functions#getter-functions) `dnsresolve(){:tact}`, которая соответствует его [варианту FunC](https://docs.ton.org/develop/howto/subresolvers#dnsresolve-code).
