...
Предоставляет [типажи](/book/types#composite-types), позволяющие остановить [контракт](/book/contracts). Полезно для аварийных режимов или режимов обслуживания. Требует типаж [`Ownable{:tact}`](/ref/stdlib-ownable#ownable) из [`@stdlib/ownable`](/ref/stdlib-ownable). Этот типаж управляет одним флагом `stopped` в контракте, а обработка состояния остановки должна выполняться в самом контракте.
...
[Типаж](/book/types#composite-types) `Stoppable{:tact}` реализует приемник для [сообщения](/book/structs-and-messages#messages) [строку](/book/types#primitive-types) "Stop", который может быть отправлен владельцем, реализует `stopped(){:tact}` [функцию получения](/book/functions#getter-functions), которая возвращает `true{:tact}`, если контракт остановлен (или `false{:tact}` в противном случае), и предоставляет частные (не получающие) функции `requireNotStopped(){:tact}` и `requireStopped(){:tact}`.
...
`Resumable{:tact}` [типаж](/book/types#composite-types) расширяет черту [`Stoppable{:tact}`](#stoppable) и позволяет возобновить выполнение [контракта](/book/contracts).
