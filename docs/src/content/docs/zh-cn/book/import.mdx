...
但要使用此类文件中的函数，必须先将它们声明为 `native` 函数。 但要使用此类文件中的函数，必须先将它们声明为 `native` 函数。 例如，当标准库 [@stdlib/dns](/zh-cn/ref/stdlib-dns) 使用 `dns.fc` FunC 文件时，它会将 FunC 函数映射到 Tact 函数，如下所示：
