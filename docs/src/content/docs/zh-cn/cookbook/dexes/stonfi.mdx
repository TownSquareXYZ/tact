...
[STON.fi SDK](https://github.com/ston-fi/sdk) 定义了一些[常量用于处理费用](https://github.com/ston-fi/sdk/blob/786ece758794bd5c575db8b38f5e5de19f43f0d1/pubes/sdk/src/contracts/dex/v2_1/router/BaseRouterV2_1.s)。 请注意这些是硬编码值，但最佳做法是[用当前配置参数动态计算费用](https://docs.ton.org/v3/guidelines/smart-contracts/fee-calculation) 请注意这些是硬编码值，但最佳做法是[用当前配置参数动态计算费用](https://docs.ton.org/v3/guidelines/smart-contracts/fee-calculation)
...
:::caution
  ...
  此外，有些变量，如`offerAmount`，为了演示目的而硬编码。 别忘了在真实生活的场景中更改它们。 别忘了在真实生活的场景中更改它们。
...
要将 Toncoin 兑换为 Jetton，STON.fi 需要使用一种被称为代理 Toncoin 钱包（简称 pTON）的工具。 要与它进行适当的互动，我们需要引入一个 `ProxyToncoinTransfer{:tact}` [Message][message] 要与它进行适当的互动，我们需要引入一个 `ProxyToncoinTransfer{:tact}` [Message][message]
...
STON.fi 允许您只指定一种代币来存入流动性--池将自动执行交换并铸造流动性提供者（LP）代币。 要做到这一点，您需要将 `ProvideLiquidity{:tact}` [Message][message] 的 `bothPositive` 字段设置为 `false{:tact}`。 要做到这一点，您需要将 `ProvideLiquidity{:tact}` [Message][message] 的 `bothPositive` 字段设置为 `false{:tact}`。
...
要提取流动性，需要燃烧的 LP 代币。 您可以参考 [Jettons Cookbook 页面中有关 Jetton 销毁的相关部分](/cookbook/jettons#burning-jetton) 的示例。 然而，应该添加比正常销毁更多的 Toncoin，因为如果添加的 Toncoin 太少，可能会导致 LP 代币被销毁，但不会从池中发送任何（或仅部分）流动性。 因此，请考虑至少附上 $0.5$ Toncoin - 超额部分将予以退还。

:::tip[Hey there!]

找不到您最喜欢的 STON.fi 交互的例子吗？ 您有很酷的实施方案吗？ [欢迎贡献！](https://github.com/tact-lang/tact/issues) 您有很酷的实施方案吗？ [欢迎贡献！](https://github.com/tact-lang/tact/issues)
