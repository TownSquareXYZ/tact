...
支持在 **2023.** 及更高版本的 JetBrains  IDEs中突出显示 Tact 的语法。 请注意，除了对 Tact 的支持，它还包括对 TON 区块链的 FunC 和 Fift 语言以及 TL-B 模式的丰富支持。 请注意，除了对 Tact 的支持，它还包括对 TON 区块链的 FunC 和 Fift 语言以及 TL-B 模式的丰富支持。
...
4. 点击插件名称旁边的 **Install** 按钮。 等待安装完成。 等待安装完成。
5. 插件安装完成后，系统会提示您重新启动 JetBrains IDE。 单击 ** 重新启动** 按钮应用更改。 单击 ** 重新启动** 按钮应用更改。 单击 ** 重新启动** 按钮应用更改。
