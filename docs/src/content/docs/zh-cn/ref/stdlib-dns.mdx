...
规范化 [`Slice{:tact}`][slice] 的内部 DNS 表示。 规范化 [`Slice{:tact}`][slice] 的内部 DNS 表示。 传入的 [`Slice{:tact}`][slice] 不能有任何引用，否则[ext code 134](/zh-cn/book/exit-codes#134)将会抛出：`Invalid argument`。
